...
public class ApplicationLifecycleRepository {
  ...
  private static final String INSERT_HISTORY =
      ClasspathSql.load("sql/lifecycle/insert-history.sql");
  ...
  public long createApplication(
      long jobId, long candidateProfileId, LocalDate effectiveDate, String note) {
    try {
      ...
      throw new LifecycleConflictException(
          "An application already exists for this job and candidate");
    }
  }
  ...
  public ApplicationRecord findApplicationForUpdate(long id) {
    List<ApplicationRecord> applications =
        jdbc.query(
            FIND_APPLICATION_FOR_UPDATE,
            Map.of("id", id),
            (rs, rowNum) ->
                new ApplicationRecord(
                    rs.getLong("id"),
                    rs.getLong("normalized_job_id"),
  ...
  public void updateStatus(
      long id, ApplicationStatus status, LocalDate effectiveDate, String note) {
    jdbc.update(
  ...
  public void cancelStaleFollowUps(long applicationId, long currentHistoryId) {
    jdbc.update(
        CANCEL_STALE_FOLLOW_UPS,
        Map.of("applicationId", applicationId, "historyId", currentHistoryId));
  }
  ...
  public void upsertFollowUp(
      FollowUpApplication application, FollowUpPlan plan, String generationVersion) {
    jdbc.update(
        UPSERT_FOLLOW_UP,
        new MapSqlParameterSource()
            ...
            .addValue(
                "dueDate", application.statusEffectiveDate().plusDays(plan.daysAfterStatus())));
  }

  public FollowUpRecord findFollowUpForUpdate(long id) {
    List<FollowUpRecord> followUps =
        jdbc.query(
            FIND_FOLLOW_UP_FOR_UPDATE,
            Map.of("id", id),
            (rs, rowNum) ->
                new FollowUpRecord(
                    rs.getLong("id"),
