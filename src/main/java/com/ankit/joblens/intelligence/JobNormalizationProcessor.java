...
public class JobNormalizationProcessor implements ItemProcessor<RawJobPosting, NormalizedJob> {
    ...
    public JobNormalizationProcessor(List<JobPostingNormalizer> normalizers, Long failAfterItems,
    ...
    public NormalizedJob process(RawJobPosting raw) {
        if (failureInjectionEnabled && failAfterItems != null
        ...
        try {
            ...
            throw new NormalizationRejectedException(raw, "Unsupported raw job source: " + raw.source(), exception);
        }
        JobPostingNormalizer normalizer = normalizers.stream()
                .filter(candidate -> candidate.supports(source))
                .findFirst()
                .orElseThrow(() -> new NormalizationRejectedException(raw,
                        "No job posting normalizer supports source " + source));
