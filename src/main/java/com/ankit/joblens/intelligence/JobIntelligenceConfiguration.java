...
public class JobIntelligenceConfiguration {
  ...
  JobNormalizationProcessor jobNormalizationProcessor(
    ...
    boolean firstExecution =
        jobRepository.getJobExecutions(jobExecution.getJobInstance()).size() == 1;
  ...
  NormalizationFailureListener normalizationFailureListener(
      NormalizationStatusService statusService) {
  ...
  Step jobNormalizationStep(
    ...
    return new StepBuilder("jobNormalizationStep", jobRepository)
        ...
        .skipLimit(100)
        .listener(
            (org.springframework.batch.core.listener.ItemProcessListener<
                    RawJobPosting, NormalizedJob>)
                normalizationFailureListener)
        .listener(
            (org.springframework.batch.core.listener.SkipListener<RawJobPosting, NormalizedJob>)
                normalizationFailureListener)
        .build();
  }
  ...
  Step skillExtractionStep(
      JobRepository jobRepository,
      PlatformTransactionManager transactionManager,
      NormalizedJobViewReader skillExtractionReader,
      SkillExtractionProcessor skillExtractionProcessor,
      SkillExtractionWriter skillExtractionWriter,
      @Value("${joblens.intelligence.chunk-size:20}") int chunkSize) {
    return new StepBuilder("skillExtractionStep", jobRepository)
        .<NormalizedJobView, ExtractedJobSkills>chunk(chunkSize)
        .transactionManager(transactionManager)
        .reader(skillExtractionReader)
        .processor(skillExtractionProcessor)
        .writer(skillExtractionWriter)
        .build();
  }
  ...
  Step scoringStep(
      ...
      NormalizedJobViewReader scoringReader,
      ScoringProcessor scoringProcessor,
      ScoringWriter scoringWriter,
      @Value("${joblens.intelligence.chunk-size:20}") int chunkSize) {
    return new StepBuilder("scoringStep", jobRepository)
        .<NormalizedJobView, JobScore>chunk(chunkSize)
        .transactionManager(transactionManager)
        .reader(scoringReader)
