...
public class DuplicateDetectionRepository {
    ...
    private static final String UPSERT_CLUSTER = ClasspathSql.load("sql/duplicate/upsert-cluster.sql");
    ...
    private static final String DELETE_EVIDENCE = ClasspathSql.load("sql/duplicate/delete-evidence.sql");
    private static final String INSERT_EVIDENCE = ClasspathSql.load("sql/duplicate/insert-evidence.sql");
    ...
    private static final String UPSERT_SIMILARITY = ClasspathSql.load("sql/duplicate/upsert-similarity.sql");
    ...
    public List<DuplicateJobView> findJobs() {
        return jdbc.query(FIND_JOBS, Map.of(), (rs, rowNum) -> new DuplicateJobView(
                rs.getLong("id"), rs.getString("source"), rs.getString("external_job_id"),
                rs.getString("title"), rs.getString("company"), rs.getString("location"),
                rs.getString("description_text"), rs.getString("employment_type"),
    ...
    public List<DuplicateEvidence> findEvidence(long clusterId) {
        return jdbc.query(FIND_EVIDENCE, Map.of("clusterId", clusterId), (rs, rowNum) ->
                new DuplicateEvidence(rs.getLong("left_job_id"), rs.getLong("right_job_id"),
                        rs.getString("evidence_type"), rs.getString("evidence_value")));
    }
    ...
    public Map<Long, Long> findExactMemberships() {
        ...
        for (Map<String, Object> row : rows) {
            memberships.put(((Number) row.get("normalized_job_id")).longValue(),
    ...
    public void reconcileSimilarities(String algorithmVersion, List<JobSimilarity> similarities) {
        ...
        if (pairKeys.isEmpty()) {
            ...
            jdbc.update(DELETE_OBSOLETE_SIMILARITIES, new MapSqlParameterSource()
    ...
    private static MapSqlParameterSource evidenceParameters(long clusterId, DuplicateEvidence evidence) {
