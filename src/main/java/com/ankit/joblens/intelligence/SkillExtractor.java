...
public class SkillExtractor {
    ...
    public SkillExtractor(SkillCatalogService catalog) { this.catalog = catalog; }
    public ExtractedJobSkills extract(long id, String hash, String title, String description) {
        ...
        for (var entry : catalog.load().entrySet()) {
            ...
            Pattern pattern = Pattern.compile("(?<![A-Za-z0-9+#])" + Pattern.quote(phrase) + "(?![A-Za-z0-9+#])", Pattern.CASE_INSENSITIVE);
            var matcher = pattern.matcher(text);
            int count = 0;
            while (matcher.find()) count++;
            if (count > 0) found.put(entry.getValue().id(), new ExtractedJobSkills.SkillMatch(entry.getValue().id(), entry.getValue().name(), count, matcherEvidence(text, pattern)));
        }
    ...
    private static String matcherEvidence(String text, Pattern pattern) {
        var m = pattern.matcher(text); return m.find() ? m.group() : "";
    }
}
