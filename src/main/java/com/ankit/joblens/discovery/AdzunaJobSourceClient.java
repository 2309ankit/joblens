...
public class AdzunaJobSourceClient implements JobSourceClient {
    ...
    public AdzunaJobSourceClient(WebClient.Builder webClientBuilder, ObjectMapper objectMapper,
    ...
    public JobPage search(SearchProfile profile, PageRequest request) {
        ...
        if (profile.sourceKey() == null || profile.sourceKey().isBlank()) {
            throw new JobSourceException("Adzuna profile " + profile.profileId() + " has no country source_key");
        }

        Mono<String> call = webClient.get()
                .uri(uriBuilder -> uriBuilder
                        .pathSegment("jobs", profile.sourceKey().toLowerCase(), "search", Integer.toString(request.page()))
                        ...
                        .queryParamIfPresent("where", java.util.Optional.ofNullable(profile.location()))
        ...
        if (properties.retryAttempts() > 1) {
            call = call.retryWhen(Retry.backoff(properties.retryAttempts() - 1, properties.retryBackoff())
        ...
        try {
            ...
            throw new JobSourceException("Adzuna request failed for profile " + profile.profileId()
                    + " page " + request.page(), exception);
        }
        if (body == null) {
            throw new JobSourceException("Adzuna returned no response body for profile " + profile.profileId());
        }
    ...
    private Mono<String> handleResponse(HttpStatusCode statusCode, Mono<String> body) {
        ...
        if (statusCode.value() == 429 || statusCode.value() == 500 || statusCode.value() == 503) {
            return body.defaultIfEmpty("")
                    .flatMap(ignored -> Mono.error(new TransientJobSourceException(
                            "Transient Adzuna HTTP status " + statusCode.value())));
        }
        return body.defaultIfEmpty("")
                .flatMap(ignored -> Mono.error(new JobSourceException(
    ...
    private JobPage parse(String body, PageRequest request) {
        try {
            ...
            if (results == null || !results.isArray()) {
                throw new MalformedJobSourceResponseException("Adzuna response is missing the results array");
            }
