...
public class JobDiscoveryTasklet implements Tasklet {
    ...
    public JobDiscoveryTasklet(DiscoveryPersistenceService persistence, List<JobSourceClient> clients,
            AdzunaProperties properties, String requestedProfileId) {
    ...
    public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) {
        ...
        if (currentProfileId == null) {
            ...
            FetchRunState fetchRun = persistence.startOrResumeFetchRun(profile, jobInstanceId, jobExecutionId);
            ...
            if (profile == null || !profile.profileId().equals(currentProfileId)) {
                throw new IllegalStateException("Active discovery profile no longer exists: " + currentProfileId);
            }
        ...
        try {
            log.info("Fetching source={} profile={} page={} jobExecutionId={}",
                    source, profile.profileId(), nextPage, jobExecutionId);
            ...
            boolean complete = page.jobs().isEmpty() || !page.hasMore() || nextPage >= properties.maxPages();
            if (complete) {
                ...
                log.info("Completed source={} profile={} pagesBound={} jobExecutionId={}",
                        source, profile.profileId(), properties.maxPages(), jobExecutionId);
            ...
            log.warn("Failed source={} profile={} page={} jobExecutionId={} reason={}",
                    source, profile.profileId(), nextPage, jobExecutionId, failure.getMessage());
            throw failure;
        }
    }
}
