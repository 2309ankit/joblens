...
public class WeeklyMarketInsightRepository {
    ...
    public WeeklyMarketInsightRepository(JdbcTemplate jdbc) { this.jdbc = jdbc; }
    public void generate(LocalDate weekStart) {
        jdbc.update("""
            ...
            """, Date.valueOf(weekStart), weekStart.atStartOfDay(), weekStart.plusDays(7).atStartOfDay());
    }
    public List<WeeklyMarketInsight> find(LocalDate from, LocalDate to) {
        return jdbc.query("SELECT id,week_start,source,job_count,company_count,remote_job_count,average_salary,generated_at FROM weekly_market_insight WHERE week_start BETWEEN ? AND ? ORDER BY week_start DESC,source", (rs,n)->new WeeklyMarketInsight(rs.getLong(1),rs.getDate(2).toLocalDate(),rs.getString(3),rs.getInt(4),rs.getInt(5),rs.getInt(6),rs.getBigDecimal(7),rs.getObject(8,java.time.OffsetDateTime.class)), Date.valueOf(from), Date.valueOf(to));
    }
}
