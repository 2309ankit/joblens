...
class JobDiscoveryIntegrationTests {
  ...
  void fetchesMultiplePagesUntilEmptyPage() throws Exception {
    ...
    ADZUNA.enqueue(
        json(200, responseWithoutCount(job("J1", "One", 1, true), job("J2", "Two", 2, true))));
    ...
    assertThat(
            jdbcTemplate.queryForMap(
                "SELECT pages_fetched, records_received FROM source_fetch_run"))
  ...
  void ignoresInactiveProfileWithoutCallingSource() throws Exception {
    ...
    assertThat(execution.getStatus()).isEqualTo(BatchStatus.COMPLETED);
    assertThat(jdbcTemplate.queryForObject("SELECT count(*) FROM source_fetch_run", Integer.class))
        .isZero();
    assertThat(ADZUNA.takeRequest(50, java.util.concurrent.TimeUnit.MILLISECONDS)).isNull();
  }
  ...
  void unchangedAndChangedPayloadAreIdempotentAndPreserveFirstSeen() throws Exception {
    ...
    var same =
        jdbcTemplate.queryForMap(
            """
                ...
                FROM raw_job_posting WHERE external_job_id='J4'
                """);
    String unchangedProcessingStatus =
    ...
    assertThat(
            jdbcTemplate.queryForObject(
                ...
                String.class))
        .isEqualTo("NEW");
    assertThat((Timestamp) same.get("last_seen_at"))
        .isAfter((Timestamp) before.get("last_seen_at"));
    ...
    assertThat(after)
        ...
        .containsEntry("payload_hash", hash(changed));
  }
  ...
  void malformedResponseFailsJobAndFetchRun() throws Exception {
    ...
    assertThat(
            jdbcTemplate.queryForObject(
                "SELECT failure_reason FROM source_fetch_run", String.class))
  ...
  void restartsSameInstanceAtFailedPageWithoutReplayingCommittedPages() throws Exception {
    ...
    ADZUNA.enqueue(
        json(200, response(10, job("R3", "Three", 3, true), job("R4", "Four", 4, true))));
    ...
    assertThat(
            failed
                ...
                .next()
                .getExecutionContext()
                .getInt(JobDiscoveryTasklet.NEXT_PAGE))
        .isEqualTo(3);
  ...
  private static String job(String id, String title, int salary, boolean customValue) {
    return "{\"id\":\""
        + id
        + "\",\"redirect_url\":\"https://example/jobs/"
        + id
        + "\",\"title\":\""
        + title
        + "\",\"salary_min\":"
  ...
  private static String hash(String rawJson) throws Exception {
    return HexFormat.of()
        .formatHex(
            MessageDigest.getInstance("SHA-256").digest(rawJson.getBytes(StandardCharsets.UTF_8)));
  }
}
