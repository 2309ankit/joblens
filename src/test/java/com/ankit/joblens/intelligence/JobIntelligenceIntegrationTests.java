...
class JobIntelligenceIntegrationTests {
    ...
    void cleanAndCreateRawInfrastructure() throws Exception {
        ...
        jdbcTemplate.update("""
                ...
                """);
        sourceFetchRunId = jdbcTemplate.queryForObject("""
                ...
                RETURNING id
                """, Long.class, infrastructureExecution.getJobInstanceId(), infrastructureExecution.getId());
        discoveryExecutionId = infrastructureExecution.getId();
    }

    @Test
    void normalizesValidAdzunaPostingAndPersistsAllFields() throws Exception {
        long rawId = insertRaw("A1", validJob("A1", "Senior Java Engineer", "<p>Java &amp; Spring</p>"));
    ...
    void rejectsMissingTitleAndNonObjectRawJsonWithReasons() throws Exception {
        ...
        assertThat(jdbcTemplate.queryForObject("SELECT count(*) FROM normalized_job", Integer.class)).isZero();
    ...
    void completedRowsAreNotReprocessedOnAnotherJobInstance() throws Exception {
        ...
        var updatedAt = jdbcTemplate.queryForObject(
                ...
                java.sql.Timestamp.class, rawId);
        ...
        assertThat(jdbcTemplate.queryForObject("SELECT count(*) FROM normalized_job", Integer.class)).isEqualTo(1);
        assertThat(jdbcTemplate.queryForObject(
                "SELECT updated_at FROM normalized_job WHERE raw_job_posting_id = ?",
                java.sql.Timestamp.class, rawId)).isEqualTo(updatedAt);
    }
    ...
    void changedRawPayloadBecomesEligibleAndUpdatesNormalizedRow() throws Exception {
        ...
        var createdAt = jdbcTemplate.queryForObject(
                ...
                java.sql.Timestamp.class, rawId);
        var firstUpdatedAt = jdbcTemplate.queryForObject(
                "SELECT updated_at FROM normalized_job WHERE raw_job_posting_id = ?",
                java.sql.Timestamp.class, rawId);
        ...
        var row = jdbcTemplate.queryForMap("""
                ...
                FROM normalized_job WHERE raw_job_posting_id = ?
                """, rawId);
        assertThat(row).containsEntry("title", "Changed Title")
    ...
    void restartResumesAfterCommittedChunkWithoutDuplicates() throws Exception {
        ...
        assertThat(jdbcTemplate.queryForObject("SELECT count(*) FROM normalized_job", Integer.class)).isEqualTo(2);
        assertThat(jdbcTemplate.queryForObject(
                "SELECT processing_status FROM raw_job_posting WHERE id = ?", String.class, failedId))
                .isEqualTo("FAILED");
        assertThat(failed.getStepExecutions().iterator().next().getExecutionContext()
                .getLong(RawJobPostingReader.LAST_COMMITTED_ID)).isEqualTo(secondId);
        ...
        assertThat(jdbcTemplate.queryForObject("SELECT count(*) FROM normalized_job", Integer.class)).isEqualTo(5);
        assertThat(jdbcTemplate.queryForObject("""
                SELECT count(*) FROM (
    ...
    private long insertRaw(String externalId, String rawJson) throws Exception {
        return jdbcTemplate.queryForObject("""
                ...
                sha256(rawJson), rawJson, discoveryExecutionId);
    }
