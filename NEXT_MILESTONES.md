...
### Semantic skill extraction (implemented, shipped disabled)
...
content patch (migration `V28`) hand-seeded missing terms but didn't fix the underlying exact-phrase-
only matching. The owner selected this as the module for the 2026-09-11 session; full design record,
implementation, test evidence, and the real Alpine/glibc deployment blocker that led to shipping it
disabled by default are all in
[S0_4_SEMANTIC_SKILL_EXTRACTION.md](S0_4_SEMANTIC_SKILL_EXTRACTION.md). Do not re-litigate the design
decisions recorded there (ONNX runtime library, catalog-embedding cache strategy, auto-accept risk
tolerance) — they're settled; what remains is the base-image decision and fixture-sweep threshold
calibration listed in that document's §8.
