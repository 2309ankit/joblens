...
function Dashboard() {
  ...
  async function run(action, success) {
    ...
    catch (failure) { setError(displayMessage(failure)); }
  ...
  return <main className="app-shell">
    ...
    {notice && <p className="toast success" role="status">✦ {notice}</p>}{error && <p className="toast error" role="alert">{error}</p>}
    <section className="hero surface"><div className="hero-copy"><p className="eyebrow">Your opportunity cockpit</p><h1>Make the next move<br /><em>intentional.</em></h1><p>JobLens turns your chosen direction into an explainable, ranked job market view.</p><div className="hero-actions"><button className="button primary" disabled={working} onClick={() => run(findJobs, 'Your Find Jobs run has started.')}>{working ? 'Scanning markets…' : 'Find fresh opportunities'} <span aria-hidden="true">→</span></button><a className="button ghost" href="#ranked-jobs">View rankings</a></div></div><div className="hero-orb" aria-hidden="true"><span className="orb-core">{jobCount}<small>ranked</small></span><i /><b /><em /></div></section>
...
function Brand({ compact = false }) { return <a className={`brand ${compact ? 'compact' : ''}`} href="/dashboard" aria-label="JobLens dashboard"><span className="brand-mark">JL</span><span>JobLens<small>career intelligence</small></span></a>; }
...
function EmptyJobs() { return <div className="empty-jobs"><span>✦</span><h3>Your first ranked opportunities will land here.</h3><p>Complete your profile, then run a search to map the market around your goals.</p><a className="button secondary" href="/setup">Refine profile</a></div>; }
